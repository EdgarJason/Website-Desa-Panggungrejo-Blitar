      ...
      <div className="w-full lg:w-2/3 flex flex-col relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          {filosofiElements.map((item) => (
            <div 
              key={item.id} 
              className="group relative flex lg:block overflow-hidden bg-white lg:bg-gray-50 rounded-2xl lg:aspect-square p-5 lg:p-0 gap-5 items-center border border-gray-100 shadow-sm lg:shadow-none hover:shadow-lg transition-all duration-300"
            >
              {/* Icon */}
              <div className="lg:absolute lg:inset-0 lg:flex lg:items-center lg:justify-center shrink-0">
                <Image 
                  src={item.src} 
                  alt={item.title} 
                  width={120} 
                  height={120} 
                  className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-16 lg:h-16 xl:w-24 xl:h-24 object-contain transition-all duration-500 lg:group-hover:scale-110 lg:group-hover:blur-[3px] drop-shadow-sm" 
                />
              </div>
              
              {/* Text Overlay / Mobile Text */}
              <div className="lg:absolute lg:inset-0 lg:bg-black/80 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-500 lg:flex lg:flex-col lg:items-center lg:justify-center lg:p-4 text-left lg:text-center z-10 lg:backdrop-blur-[2px]">
                <div className="flex flex-col lg:items-center">
                  <h3 className="font-bold text-gray-900 lg:text-white mb-1.5 text-base md:text-sm lg:text-sm xl:text-base lg:-translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-500 ease-out lg:drop-shadow-md">
                    {item.title}
                  </h3>
                  <p className="text-gray-600 lg:text-gray-200 text-xs sm:text-sm md:text-[11px] lg:text-[11px] xl:text-xs lg:translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-500 ease-out leading-relaxed lg:leading-tight xl:leading-snug">
                    {item.description}
                  </p>
                </div>
              </div>
